...
class AdaptVQE(VariationalAlgorithm, MinimumEigensolver):
    ...
    def compute_minimum_eigenvalue(
        ...
        while self.max_iterations is None or iteration < self.max_iterations:
            ...
            #sorted_indices = np.argsort(-np.abs(grads))
            #found_new_op = False
            ...
            cur_grads_copy = cur_grads.copy()
            cur_grads_new = []
            for g in range(len(cur_grads_copy)):
                cur_grads_new.append(cur_grads_copy[g][0])

            sorted_indices = np.argsort(-np.abs(cur_grads_new))
            print(cur_grads_new)
            found_new_op = False
            for idx in sorted_indices:
                evo_op = self._excitation_pool[idx]
                #self._excitation_pool[max_grad_index]
                if evo_op not in selected_ops:
                    max_grad_index = idx
                    #max_grad = np.abs(cur_grads_new[max_grad_index])
                    print('max_grad=',np.abs(cur_grads_new[max_grad_index]))
                    selected_ops.append(evo_op)
                    found_new_op = True
                    break

            if not found_new_op:
                logger.info("No new operator found. Terminating.")
                termination_reason = TerminationCriterion.CYCLICITY
                break
