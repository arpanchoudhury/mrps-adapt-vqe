...
from qiskit_algorithms.minimum_eigensolvers import MinimumEigensolver, MinimumEigensolverResult, VQE, VQEResult
...
logger.info("Test INFO log prints")
...
class AdaptVQE(VariationalAlgorithm, MinimumEigensolver):
    ...
    def __init__(
        ...
        *,
        ...
        original_excitations_list: list,
        #preset_amplitudes_dict: dict,
        #dmrg_statevector: Statevector
        ...
        self._excitation_pool = original_excitations_list
        #self._preset_amplitudes = preset_amplitudes_dict
        #self._dmrg_statevector = dmrg_statevector
    ...
    def compute_minimum_eigenvalue(
        ...
        #print('operators from source', self._excitation_pool)
        ...
        while self.max_iterations is None or iteration < self.max_iterations:
            ...
            print("Found maximum gradient",str(np.abs(max_grad[0])))
            # log gradients
            ...
            #logger.info("Adding new operator to the ansatz: %s", str(self._excitation_pool[max_grad_index]))
            
            '''newly_added_excitation = self._excitation_pool[max_grad_index]
            print(f"[Iteration {iteration}] Adding excitation operator: {newly_added_excitation}")


            # Try to extract excitation indices if possible
            # For FermionicOp from Qiskit Nature, excitation indices may be in `label` or `display_name`
            if hasattr(newly_added_excitation, 'label'):
                print(f"  Operator label: {newly_added_excitation.label}")
            elif hasattr(newly_added_excitation, 'display_name'):
                print(f"  Display name: {newly_added_excitation.display_name}")
            else:
                print(f"  Operator str: {str(newly_added_excitation)}")'''

            print(f"Iteration {iteration}: Adding excitation index = {max_grad_index}")
            # If using the indexed pool (tuples of (excitation indices, FermionicOp))
            #selected_exc = self._original_excitations[max_grad_index]
            print(f"Iteration {iteration}: Adding operator = {self._excitation_pool[max_grad_index]}")

            #print(f"[Iteration {iteration}] Adding excitation: {selected_exc}")
            #print(f"Corresponding FermionicOp: {selected_op}")
            self._excitation_list.append(self._excitation_pool[max_grad_index])

            # add DMRG amplitudes here instead of 0.0
            #newly_added_amplitude = self._preset_amplitudes.get(selected_exc)
            #print('newly_added_amplitude',newly_added_amplitude)
            theta.append(0.0) # change this
            #theta.append(newly_added_amplitude)
            ...
            # Generate circuit with current optimal parameters
            inter_params_opt = list(raw_vqe_result.optimal_point)
            current_circuit = self.solver.ansatz.assign_parameters(inter_params_opt)

            #print('Number of gates = ', current_circuit.decompose().decompose().decompose().count_ops(), flush=True)
            #current_circuit = self.solver.ansatz.bind_parameters(raw_vqe_result.optimal_point)
            # Simulate to get statevector
            #statevector = Statevector.from_instruction(current_circuit)

            from qiskit.quantum_info import state_fidelity
            #current_fidelity = state_fidelity(statevector,self._dmrg_statevector)
            #print('Fidelity =',current_fidelity)
            #statevector_history.append(statevector)
        ...
        #result.statevector_history = statevector_history  # type: ignore[attr-defined]
...
class AdaptVQEResult(VQEResult):
    ...
    def __init__(self) -> None:
        ...
        self._statevector_history: list[np.ndarray] = []  # <-- new
    ...
    @property
    def statevector_history(self) -> list[np.ndarray]:
        """Returns the history of statevectors at each iteration."""
        return self._statevector_history

    @statevector_history.setter
    def statevector_history(self, statevector_history: list[np.ndarray]) -> None:
        """Sets the history of statevectors."""
        self._statevector_history = statevector_history
